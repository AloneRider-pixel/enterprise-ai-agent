...
def analyze(snapshot: PullRequestSnapshot) -> tuple[list[Finding], list[RiskFactor]]:
    ...
    security_paths = _paths_matching(paths, SECURITY_PATHS)
    if security_paths:
        findings.append(
            Finding(
    ...
    infra_paths = _paths_matching(paths, INFRA_PATHS)
    if infra_paths:
        findings.append(
            Finding(
    ...
    data_paths = _paths_matching(paths, DATA_PATHS)
    if data_paths:
        findings.append(
            Finding(
                severity=Severity.MEDIUM,
    ...
    dependency_paths = _paths_matching(paths, DEPENDENCY_PATHS)
    if dependency_paths:
        findings.append(
            Finding(
                severity=Severity.MEDIUM,
    ...
    test_paths = _paths_matching(paths, TEST_PATHS)
